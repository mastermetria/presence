...
def a1():
    if request.method == 'POST':
        ...
        if user_input is None:
            return "No input received", 400  # Retourne un message d'erreur si aucun input n'est reçu

        thread = threading.Thread(target=run, args=(user_input,))
        thread.start()
        
        return redirect('/a1')  # Retourne toujours une réponse en chaîne de caractères
    else:
        pdf_list = os.listdir('automations/a1/downloads')
        return render_template('a1/interface.html', pdf_list=pdf_list)


@app.route('/a1/get_pdfs')
def get_pdfs():
    # Obtenir la liste des fichiers PDF dans le dossier
    pdf_files = [f for f in os.listdir('automations/a1/downloads') if f.endswith('.pdf')]
